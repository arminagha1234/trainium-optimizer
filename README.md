...
- **The kernel stage is now wired end-to-end in code — but two links are unproven.** The full pipeline exists and is unit-tested: route (`kernel_registry`/`preflight`, wired into `overnight`) → escalation ladder (rewrite catalog incl. `.tril`→const-mask and `sort`→argmax; rank-aware harvest/reuse) → author (recipe table today, with a pluggable **LLM-author seam**, `kernel_author.py`) → **repair loop** (feeds the exact compiler error back, wired into `invent_engine.run_op`) → tiered validation (**rank ladder** with the `simulate ≠ on-device` wall + an **adversarial anti-reward-hacking gate**) → **generic serving-injection hook** (`kernel_inject`) → persist + author-time bank retrieval. The two links **not yet proven**: (1) the injection hook + kernel execution are **not validated on real hardware** (mock-tested only — needs an on-device window), and (2) the LLM author is a **seam with no real provider wired** (the recipe table is the default). So a kernel win still isn't *measured* end-to-end yet. Most auto-authored kernels are expected to lose to the compiler anyway; the value is the occasional win + the accumulated lessons. See [`docs/kernel-stage-deepdive.md`](./docs/kernel-stage-deepdive.md), [`docs/kernel-stage-external-prior-art.md`](./docs/kernel-stage-external-prior-art.md), and [`docs/nki-optimization-playbook.md`](./docs/nki-optimization-playbook.md).
