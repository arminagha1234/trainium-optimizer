...
- Two tiers: `provisional/` (auto-generated) and `verified/` (signed-off)
- Proposer only pulls from `verified/`. `provisional/` is a review queue.
- Human verification is: read the lesson, decide if it generalizes, promote
  or drop (`bank.promote(lesson_id)`).

#### Autonomous promotion (for no-human-in-the-loop runs)

The `verified`-only rule creates a problem for an overnight run with no human
watching: the loop *writes* provisional lessons but never gets to *reuse* them,
so seeds 1-3 each re-derive the same wins and the bank never compounds within
the run. `AutoPromotionPolicy` closes that loop — it promotes
`provisional → verified` automatically, but only against a deliberately
conservative, **explicit** bar (a bad auto-promotion pollutes every later
model's search):

- validated on ≥ `min_models` distinct models,
- across ≥ `min_families` architecture families,
- every recorded measurement cleared `min_correctness` (99%),
- invented kernels additionally beat their borrowed alternative by ≥ the
  invention margin (5%) — no "invented" freebies.

The bar is expressed as those interpretable criteria, **not** as
`Confidence.score()` — that score is calibrated for *ranking* verified lessons
(it returns 0.2 for a solid 2-model lesson) and would fight `min_models` if
used as a gate.

Two audit safeguards keep it honest:

- Auto-promoted lessons are stamped `auto_promoted: true` + `promoted_at`, and
  are **not** marked `human_verified`. So `human_verified_ratio` still reflects
  only human sign-off, and `stats()["auto_promoted"]` shows exactly how much of
  the bank the autonomous loop now owns — the number to watch and spot-audit.
- Off by default (humans-only). `AutoPromotionPolicy.overnight()` is the opt-in
  preset for the autonomous run; it relaxes `min_families` to 1 so a lesson
  proven on two models of the *same* family can compound within the run.
