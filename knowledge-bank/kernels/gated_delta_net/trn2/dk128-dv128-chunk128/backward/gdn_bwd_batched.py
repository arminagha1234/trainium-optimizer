"""gdn_bwd_batched.py — full GDN chunked backward, batched over (batch, head).

Wraps the on-device-validated single-(B,H) backward (gdn_bwd_full_nki) with an
outer sequential_range(BH) — matches the fwd kernel's dispatch-amortization idiom
(one launch handles all BH slices sequentially, saving Python/PyTorch per-call
overhead). Inputs mirror the fwd kernel's [BH,...] layout.
"""
...
@nki.jit
def gdn_bwd_batched(q, k, v, g, beta, S0, dO,
                    tril_incl, strict, eye, triu_incl, last_mask):
    BH = q.shape[0]; T = q.shape[1]; n = T // C
    dq_out = nl.ndarray((BH, T, DK), dtype=nl.float32, buffer=nl.shared_hbm)
    dk_out = nl.ndarray((BH, T, DK), dtype=nl.float32, buffer=nl.shared_hbm)
    dv_out = nl.ndarray((BH, T, DV), dtype=nl.float32, buffer=nl.shared_hbm)
    dg_out = nl.ndarray((BH, T, 1), dtype=nl.float32, buffer=nl.shared_hbm)
    dbeta_out = nl.ndarray((BH, T, 1), dtype=nl.float32, buffer=nl.shared_hbm)
    dS0_out = nl.ndarray((BH, DK, DV), dtype=nl.float32, buffer=nl.shared_hbm)
    ...
    for b in nl.sequential_range(BH):
        ...
        for i in nl.static_range(n):
            cs = i * C
            kt = nl.load(k[b, cs:cs + C, :]); vt = nl.load(v[b, cs:cs + C, :])
            gcol = nl.load(g[b, cs:cs + C, :]); bcol = nl.load(beta[b, cs:cs + C, :])
        ...
        for jj in nl.static_range(n):
            i = n - 1 - jj
            cs = i * C
            qt = nl.load(q[b, cs:cs + C, :]); kt = nl.load(k[b, cs:cs + C, :]); vt = nl.load(v[b, cs:cs + C, :])
            gcol = nl.load(g[b, cs:cs + C, :]); bcol = nl.load(beta[b, cs:cs + C, :]); dot = nl.load(dO[b, cs:cs + C, :])
            St = S_list[i]
            grow = nisa.tensor_copy(nisa.nc_transpose(gcol))
            gcrow = nisa.tensor_tensor_scan(ones_1c, grow, zero_11, nl.multiply, nl.add)
            ...
            dg = _mm(triu, dgc)
            nisa.dma_copy(dst=dq_out[b, cs:cs + C, :], src=nisa.tensor_copy(dq))
            nisa.dma_copy(dst=dk_out[b, cs:cs + C, :], src=nisa.tensor_copy(dk))
            nisa.dma_copy(dst=dv_out[b, cs:cs + C, :], src=nisa.tensor_copy(dv))
            nisa.dma_copy(dst=dg_out[b, cs:cs + C, :], src=nisa.tensor_copy(dg))
            nisa.dma_copy(dst=dbeta_out[b, cs:cs + C, :], src=nisa.tensor_copy(dbeta))
            dS = dS_in
        nisa.dma_copy(dst=dS0_out[b, :, :], src=nisa.tensor_copy(dS))
