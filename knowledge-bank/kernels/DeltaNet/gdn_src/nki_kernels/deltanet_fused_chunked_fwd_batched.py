# =============================================================================
# gdn-backward project -- v3 BATCHED forward kernel (Task 014)
# =============================================================================
# Sourced from `deltanet_fused_chunked_v2.py`.
# Extensions:
#   - Adds outer BH dimension to all HBM tensors: (BH, S, D), (BH, D, D), etc.
#   - Wraps entire body in `for sample_idx in nl.sequential_range(BH)`.
#   - One kernel launch handles all (batch, head) slices in sequence.
#
# Motivation: dispatching once instead of BH times amortizes Python/PyTorch
# per-call overhead (~700us per call at seq=1024 wall-clock).
# =============================================================================
...
@nki.jit
def deltanet_fused_chunked_fwd_batched(
    query: nl.ndarray,  # (BH, S, 128) float32 -- l2-normed and scaled
    key: nl.ndarray,  # (BH, S, 128) float32 -- l2-normed
    value: nl.ndarray,  # (BH, S, 128) float32
    g_in: nl.ndarray,  # (BH, S, 1)   float32 -- per-token log-decay (NOT cumsum)
    beta_in: nl.ndarray,  # (BH, S, 1)   float32 -- per-token write gate
    initial_state: nl.ndarray,  # (BH, 128, 128) float32 -- initial recurrent state (pass zeros if unused)
    ...
    state (128x128) in SBUF across chunks.  Returns per-token output, final state,
    and per-chunk saved intermediates (N and w) for use by the backward kernel.
    ...
    Input requirements:
      ...
      - initial_state is the (128, 128) recurrent state at chunk 0 entry.
        Pass a zero tensor if the caller does not want a non-zero initial state.

    Returns:
        output:      (S, 128) float32 -- per-token output
        final_state: (128, 128) float32 -- state after final chunk
        N_stack:     (num_chunks, 128, 128) float32 -- Neumann matrix per chunk (P_acc at chunk end)
        w_stack:     (num_chunks, 128, 128) float32 -- k_cumdecay per chunk
    """
    BH = query.shape[0]
    ...
    # Output tensors in HBM (all batched with leading BH dim)
    ...
    N_stack_out = nl.ndarray(
        (BH, num_chunks, P_MAX, P_MAX), dtype=nl.float32, buffer=nl.shared_hbm
    )
    w_stack_out = nl.ndarray(
    ...
    # Sequential to allow the compiler to schedule tightly; one dispatch handles all.
    # ================================================================
    for sample_idx in nl.sequential_range(BH):
        # Initialize recurrent state in SBUF from this slice's initial_state input.
        state = nl.ndarray((P_MAX, dim), dtype=nl.float32, buffer=nl.sbuf)
        nisa.dma_copy(dst=state, src=initial_state[sample_idx, 0:P_MAX, 0:dim])
    ...
    # ================================================================
        for i_chunk in nl.sequential_range(num_chunks):
            ...
            nisa.dma_copy(
                dst=q_c,
                src=query[sample_idx, chunk_start : chunk_start + CHUNK_SIZE, 0:dim],
            )

            k_c = nl.ndarray((P_MAX, dim), dtype=key.dtype, buffer=nl.sbuf)
            nisa.dma_copy(
                dst=k_c,
                src=key[sample_idx, chunk_start : chunk_start + CHUNK_SIZE, 0:dim],
            )

            v_c = nl.ndarray((P_MAX, dim), dtype=value.dtype, buffer=nl.sbuf)
            nisa.dma_copy(
                dst=v_c,
                src=value[sample_idx, chunk_start : chunk_start + CHUNK_SIZE, 0:dim],
            )
            ...
            # ---- Compute exp(gc), exp(g_last - gc), exp(g_last) as (P_MAX, 1) ----
            # All exp() arguments are <= 0, so outputs are in (0, 1]. No overflow.
            ...
            # exp(g_last - gc): g_last <= gc[t] for all t, so arg <= 0. SAFE.
            ...
            # ============================================================
            # Build stable decay mask: exp(gc[i] - gc[j])
            # gc_diff[i,j] = gc[i] - gc[j], clamped to <= 0
            # ============================================================
            ones_PP = nl.ndarray((P_MAX, P_MAX), dtype=nl.float32, buffer=nl.sbuf)
            nisa.memset(dst=ones_PP, value=1.0)

            # gc_mat_rows[i,j] = gc[i] — broadcast gc_p across columns
            ...
            # gc_mat_cols[i,j] = gc[j] — broadcast gc_row across rows
            ...
            # Clamp to <= 0 to avoid exp overflow in upper triangle
            ...
            # exp_gc_diff[i,j] = exp(gc[i] - gc[j]), all in (0, 1]
            ...
            # C1 (task 004 fusion): PSUM-operand fuse -- consume QK_psum directly in
            # the elementwise multiply below. No intermediate PSUM->SBUF tensor_copy.
            QK_psum = nl.ndarray((P_MAX, P_MAX), dtype=nl.float32, buffer=nl.psum)
            nisa.nc_matmul(dst=QK_psum, stationary=kb_T, moving=k_T)

            # QK_decay = QK * exp_gc_diff (element-wise, stable) -- PSUM operand
            QK_decay = nl.ndarray((P_MAX, P_MAX), dtype=nl.float32, buffer=nl.sbuf)
            nisa.tensor_tensor(dst=QK_decay, data1=QK_psum, data2=exp_gc_diff, op=nl.multiply)
            ...
            # v2: save N (= P_acc, the Neumann matrix) and w (= k_cumdecay) to HBM
            # for use by the backward kernel.
            nisa.dma_copy(
                dst=N_stack_out[sample_idx, i_chunk, 0:P_MAX, 0:P_MAX], src=P_acc
            )
            nisa.dma_copy(
                dst=w_stack_out[sample_idx, i_chunk, 0:P_MAX, 0:dim], src=k_cumdecay
            )
            ...
            # Apply stable decay mask -- C3 (task 004 fusion): PSUM-operand fuse.
            qk_decay = nl.ndarray((P_MAX, P_MAX), dtype=nl.float32, buffer=nl.sbuf)
            nisa.tensor_tensor(
                dst=qk_decay, data1=qk_psum, data2=exp_gc_diff, op=nl.multiply
            )
            ...
            # v_new = value_corr - v_prime -- C4 (task 004 fusion): PSUM-operand fuse.
            v_new = nl.ndarray((P_MAX, dim), dtype=nl.float32, buffer=nl.sbuf)
            nisa.tensor_tensor(dst=v_new, data1=value_corr, data2=vp_psum, op=nl.subtract)
            ...
            # ============================================================
            # chunk_output = attn_inter + intra_out -- C5 (task 004 fusion): PSUM-operand fuse.
            # ============================================================
            chunk_out = nl.ndarray((P_MAX, dim), dtype=nl.float32, buffer=nl.sbuf)
            nisa.tensor_tensor(dst=chunk_out, data1=attn_inter, data2=intra_psum, op=nl.add)

            # Store output chunk to HBM
            nisa.dma_copy(
                dst=output[sample_idx, chunk_start : chunk_start + CHUNK_SIZE, 0:dim],
                src=chunk_out,
            )

            # ============================================================
            # State update: state = exp(g_last) * state + k_state_decay^T @ v_new
            # k_state_decay = k * exp(g_last - gc)  [exp arg <= 0, safe]
            ...
            # C6 (task 004 fusion): PSUM-operand fuse -- kv_psum is consumed by the
            # tensor_tensor(add) below with an intervening tensor_scalar on `state`
            # that does not touch PSUM. Skip the intermediate SBUF copy.
            ...
            nisa.tensor_tensor(dst=state, data1=state_decayed, data2=kv_psum, op=nl.add)

        # ---- Write final state to HBM ----
        nisa.dma_copy(dst=final_state_out[sample_idx, 0:P_MAX, 0:dim], src=state)

    return output, final_state_out, N_stack_out, w_stack_out
