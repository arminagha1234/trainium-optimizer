#!/usr/bin/env bash
# Reproduce the optimized Qwen/Qwen3-32B recipe.
# Backend: native-pytorch-beta3
# Expected: 3698 tok/s (3.794x baseline)
...
python -m optimizer.apply \
    ...
    --backend native-pytorch-beta3 \
    --set tp_degree=4 --set cp_degree=2 --set batch=1 --set weights_dtype=bf16 --set compile_mode=compile-default

# Then measure to confirm you land within tolerance of 3698 tok/s.
python -m optimizer.measure --model Qwen/Qwen3-32B --backend native-pytorch-beta3 --all-shapes
