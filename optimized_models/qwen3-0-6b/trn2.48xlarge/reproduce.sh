...
# Expected: 99743 tok/s (28.916x baseline)
...
python -m optimizer.apply \
    ...
    --backend native-pytorch-beta3 \
    --set tp_degree=16 --set weights_dtype=bf16 --set attn_implementation=eager --set compile_mode=compile-default --set batch=8 --set cp_degree=1 --set dp_degree=4 --set kv_replication=1 --set cores_used=64 --set cores_available=64 --set cc_flags=--model-type transformer

# Then measure to confirm you land within tolerance of 99743 tok/s.
