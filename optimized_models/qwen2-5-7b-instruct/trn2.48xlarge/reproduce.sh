...
# Expected: 9976 tok/s (3.241x baseline)
...
python -m optimizer.apply \
    ...
    --backend native-pytorch-beta3 \
    --set tp_degree=4 --set weights_dtype=bf16 --set attn_implementation=eager --set compile_mode=eager --set batch=8 --set cp_degree=1 --set dp_degree=16 --set kv_replication=1 --set cores_used=64 --set cores_available=64

# Then measure to confirm you land within tolerance of 9976 tok/s.
